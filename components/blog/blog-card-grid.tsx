...
export function BlogCardsGrid({
  ...
  return (
    <section className={`w-full mx-auto max-w-7xl py-8 sm:py-12 md:py-16 ${className}`}>
      {/* Header */}
      {(title || subtitle) && (
      ...
      {/* Grid Container */}
      <div className="grid w-full gap-6 sm:gap-7 md:gap-8 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3">
        {data.map((item) => (
          <BlogCard
            key={item.id}
            title={item.title}
